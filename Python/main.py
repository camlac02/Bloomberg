...
if __name__ == '__main__':
    ...
    endDate = dt.datetime(2022, 12, 3)
    #bloom2 = blp.bds(strSecurity=tickers, strFields=strFields)
    #list_index = list(map(lambda x: x + " Equity", bloom2['INDX_MEMBERS'][tickers[0]]))

    #data = blp.bdh(list_index, ['CUR_MKT_CAP'], startDate, endDate) # CUR_MKT_CAP # TOT_COMMON_EQY # PX_LAST
    # data.to_csv('data_blp.csv')
    data = pd.read_csv('data_blp_mktcap.csv', index_col=0).dropna(axis=1)
    ...
    # Daily drawdown
    plt.figure(2)
    Roll_Max = back['close'].cummax()
    Daily_Drawdown = back['close'] / Roll_Max - 1.0
    Max_Daily_Drawdown = Daily_Drawdown.cummin()
    plt.plot(back['ts'], Max_Daily_Drawdown)
    plt.show()
